...
async def delete_messages():
    ...
    try:
        ...
        for line in lines:
            line = line.strip()
            if "t.me/c/" in line:
                # Обработка приватных ссылок вида t.me/c/<chat_id>/<message_id>
                ...
                if match:
                    ...
                    try:
                        ...
                        logging.info(f"✅ Удалено сообщение {message_id} из чата с ID {full_chat_id}")
                    except errors.MessageIdInvalidError:
                        logging.warning(f"⚠️ Сообщение {message_id} уже удалено или не существует.")
                    except errors.ChatAdminRequiredError:
                        logging.error(f"❌ Недостаточно прав для удаления сообщений в чате {full_chat_id}.")
